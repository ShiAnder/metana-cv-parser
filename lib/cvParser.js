...
async function extractTextFromFile(content) {
  ...
  // NEW: If content has 'extractedText' property that's a string, return that
  if (content.extractedText && typeof content.extractedText === 'string') {
  ...
  try {
    ...
    console.log('Content object structure:', JSON.stringify({
      ...
      extractedTextType: content.extractedText ? typeof content.extractedText : 'N/A'
    }));
    ...
    throw new Error('Content format not supported. Expected a string or object with content/extractedText property.');
  } catch (error) {
