...
async function saveToSheet(data) {
  try {
    ...
    console.log('Environment variable status:', {
      ...
      GOOGLE_PRIVATE_KEY: privateKey ? `present (length: ${privateKey.length})` : 'missing',
    ...
    // Remove extra quotes that might be wrapping the key
    ...
    if (!privateKey.includes('-----BEGIN PRIVATE KEY-----')) {
      throw new Error('Private key is missing the BEGIN PRIVATE KEY header');
    }
    
    if (!privateKey.includes('-----END PRIVATE KEY-----')) {
      throw new Error('Private key is missing the END PRIVATE KEY footer');
    }
